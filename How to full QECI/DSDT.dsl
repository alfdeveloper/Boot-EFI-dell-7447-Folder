 ...
 * 
 * Disassembly of DSDT.aml, Mon Aug 10 07:51:25 2015
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000C11D (49437)
 *     Revision         0x02
 *     Checksum         0x3A
 *     OEM ID           "DELL"
 *     OEM Table ID     "QA09"
...
DefinitionBlock ("DSDT.aml", "DSDT", 2, "DELL", "QA09", 0x00000034)
{
    ...
    //External (_SB_.PCI0.GFX0, MethodObj)
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xCB5B7C18, 0x02B3)
    ...
    Scope (_SB)
    {
        ...
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                Device (PS2K)
                {
                    Name (_HID, "DLLK0680")  // _HID: Hardware ID
    ...
    Scope (_SB.PCI0.LPCB)
    {
        OperationRegion (CPSB, SystemMemory, 0xCAC0AD18, 0x10)
        ...
        Device (EC)
        {
                    ...
                    While (One)
                    {
                        ...
                        Else
                        {
                            ...
                            Else
                            {
                                ...
                                Else
                                {
                                    If (LEqual (_T_0, 0x07D6))
                                    {
                                        Store (0x20, Local0)
                                    }
                                    Else
                                    {
                                        If (LEqual (_T_0, 0x07D1))
                                        {
                                            Store (0x10, Local0)
                                        }
                                        Else
                                        {
                                            If (LEqual (_T_0, 0x07D2))
                                            {
                                                Store (0x10, Local0)
                                            }
                                            Else
            ...
            Method (_Q80, 0, NotSerialized)  // _Qxx: EC Query
            {
                ...
                And (Local0, 0xFF, Local0)
                    ...
                    If (LAnd (LGreaterEqual (Local0, 0x11), LLess (
                                Local0, 0x17)))
                        {
                            Store (One, Local0)
                        }
                    If (LAnd (LGreaterEqual (Local0, 0x17), LLess (
                                Local0, 0x1D)))
                        {
                            Store (0x02, Local0)
                        }
                    If (LAnd (LGreaterEqual (Local0, 0x1D), LLess (
                                Local0, 0x23)))
                    {
                        Store (0x03, Local0)
                    }
